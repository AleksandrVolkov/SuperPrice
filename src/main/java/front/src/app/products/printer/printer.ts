...
export class PrinterComponent {
  ...
  constructor(//private keycloakService: KeycloakService,
    private httpService: BacketComponentService, private http: HttpClient,private toastr:ToastrService,public auth:AuthenticationService) {
  ...
  public ngOnInit(): void {
    this.http.get(this.URL + '/printer?page=0').subscribe((data: Page) => {
  ...
  change() {
    this.http.get(this.URL + '/printer?page=' + (this.config.currentPage - 1))
      .subscribe((data: Page) => {
        this.products = data.content;
      });
  }
  ...
  public like(item:Product): void {
    if(this.auth.isUser()){
      ...
      this.http.post(this.URL+'/printer/like',item).subscribe((data:ReturnString)=>{
...
public dislike(item:Product): void {
  if(this.auth.isUser()){
    ...
    this.http.post(this.URL+'/printer/dislike',item).subscribe((data:ReturnString)=>{
    if(data.disabledStop==true){
      item.dislike--;
      alert("Вы уже оценили этот товар");
    }
